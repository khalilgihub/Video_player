...
function getFakeMaximizedBounds(win) {
  ...
  // Blur-time DWM seams can exceed 1px on fractional scaling.
  // Push top and sides slightly off-screen to keep artifacts outside the visible work area.
  const seamTopInset = Math.max(10, Math.ceil(scaleFactor * 8));
  const seamSideInset = Math.max(3, Math.ceil(scaleFactor * 2));
  return {
    x: workArea.x - seamSideInset,
    y: workArea.y - seamTopInset,
    width: workArea.width + seamSideInset * 2,
    // Preserve bottom edge at taskbar while shifting top off-screen.
  ...
  mainWindow.once('ready-to-show', () => {
    ...
    try {
      ...
      mpvProcess.spawn(nativeHandle, {
        ...
        // Reliability-first on Windows: avoid intermittent black video output
        // seen on some GPU/driver combos with hwdec=auto-safe.
        hwdec: 'no',
